...
function PersonalManagementPage() {
    ...
    const fetchEmployees = async () => {
      try {
        setLoading(true);
        const response = await fetchWithAuth(API_ENDPOINTS.EMPLOYEE.BASE);
    ...
    const fetchDocuments = async () => {
        try {
            const response = await fetchWithAuth(API_ENDPOINTS.DOCUMENT.BASE);
    ...
    const fetchLeaveRequests = async () => {
        try {
            const response = await fetchWithAuth(API_ENDPOINTS.LEAVE_REQUEST.BASE);
    ...
    const fetchAssets = async () => {
        try {
            const response = await fetchWithAuth(API_ENDPOINTS.ASSET.BASE);
    ...
    const fetchBonuses = async () => {
        try {
            const response = await fetchWithAuth(API_ENDPOINTS.BONUS.BASE);
    ...
    const handleToggleEmployeeStatus = async (employeeId: number, isActive: boolean) => {
        try {
            ...
            const response = await fetch(API_ENDPOINTS.EMPLOYEE.STATUS(employeeId), {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const sendEmail = async (to: string, subject: string, body: string) => {
        try {
            ...
            swal("Başarılı", "E-posta başarıyla gönderildi.", "success");
        } catch (error) {
            console.error('Email gönderilemedi:', error);
            swal("Hata", "E-posta gönderilirken bir sorun oluştu.", "error");
        }
    };
    ...
    const handleDocumentUpload = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Form verisi için token ve headers
            const token = getAuthToken();
            const headers: Record<string, string> = {
                'Authorization': token ? `Bearer ${token}` : ''
            };

            const response = await fetch(API_ENDPOINTS.DOCUMENT.UPLOAD, {
                method: 'POST',
                headers,
                body: formData
            });
            ...
            swal("Hata", "Belge yüklenirken bir sorun oluştu.", "error");
        }
    };
    ...
    const handleDeleteEmployee = async (employeeId: number) => {
        try {
            ...
            if (willDelete) {
                // Gerçek API çağrısı 
                const response = await fetch(`${API_ENDPOINTS.EMPLOYEE.BY_ID(employeeId)}`, {
                    method: 'DELETE'
                });
